...
export function ShipmentsPage() {
  ...
  function advanceShipment(shipment: Shipment, stage: "out_for_delivery" | "delivered" | "delayed") {
    updateShipmentStage(shipment, stage);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[0.6875rem] font-bold uppercase tracking-[1.2px] text-[#939FAD]">Envios</p>
          <h1 className="text-xl font-bold text-[#112b4a]">
            {role === "shipper" ? "Mis entregas" : "Seguimiento de despachos"}
          </h1>
        </div>
        <div className="flex items-center gap-3 text-xs text-[#939FAD]">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar tracking, pedido, transportista..." className="h-9 w-full rounded border border-[#DDE0E2] bg-white pl-9 pr-3 text-sm outline-none placeholder:text-[#939FAD]" />
        </div>
        <div className="flex gap-1 rounded border border-[#DCE0E2] bg-white p-0.5">
          {(["all", "active", "delayed", "delivered"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={cn("rounded px-3 py-1 text-[11px] font-semibold transition-colors", filter === f ? "bg-[#4B98CF] text-white" : "text-[#939FAD] hover:text-[#112b4a]")}>
      ...
      <div className="space-y-2">
        {filtered.map((shipment) => {
          ...
          return (
            <div key={shipment.id} className="rounded border border-[#DCE0E2] bg-white p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#939FAD]">Pedido #{shipment.orderId} · SKU {shipment.sku}</p>
                  </div>
                </div>
                  ...
                  {canUpdate && shipment.stage !== "delivered" && (
                    <div className="flex items-center gap-1">
                      {!shipment.stage.includes("out") && shipment.stage !== "delayed" && (
                        <button onClick={() => advanceShipment(shipment, "out_for_delivery")} className="rounded border border-[#DCE0E2] px-2.5 py-1 text-[10px] font-semibold text-[#4B98CF] hover:bg-[#4B98CF]/5">Reparto</button>
                      )}
                      <button onClick={() => advanceShipment(shipment, "delivered")} className="rounded border border-[#DCE0E2] px-2.5 py-1 text-[10px] font-semibold text-[#4EB4A5] hover:bg-[#4EB4A5]/5">Entregado</button>
                      <button onClick={() => advanceShipment(shipment, "delayed")} className="rounded border border-[#DCE0E2] px-2.5 py-1 text-[10px] font-semibold text-red-400 hover:bg-red-50">Retraso</button>
                    </div>
                  )}
                </div>
              </div>
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded border border-[#DCE0E2] bg-white py-16">
            ...
            <p className="mt-3 text-sm font-medium text-[#939FAD]">Sin envios</p>
            <p className="mt-1 text-xs text-[#939FAD]/70">Crea pedidos y espera que el sistema genere envios automaticamente.</p>
          </div>
        )}
      </div>
    </div>
