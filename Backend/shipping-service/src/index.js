...
app.post('/api/shipments', async (req, res) => {
  try {
    ...
    if ((await pool.query('SELECT 1 FROM shipments WHERE order_id=$1', [orderId])).rows.length)
      return res.status(409).json({ error: `Ya existe envio para orden ${orderId}` });
    ...
    await sendNotification(shipment, 'SHIPMENT_CREATED', `Envio creado tracking ${tracking}`);
...
app.put('/api/shipments/:id/stage', async (req, res) => {
  try {
    ...
    if (stage === 'EN_REPARTO') {
      ...
      notifStage = 'SHIPMENT_DELIVERED'; notifMsg = `Envio entregado - ${updated.tracking_number}`;
    } else {
      updated = (await pool.query('UPDATE shipments SET status=$1 WHERE id=$2 RETURNING *', [stage, req.params.id])).rows[0];
      notifStage = 'SHIPMENT_CANCELLED'; notifMsg = `Envio cancelado - ${updated.tracking_number}`;
    }
